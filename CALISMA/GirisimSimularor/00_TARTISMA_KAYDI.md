...
**Statü:** M1 v3 "Tam Düzen" çalışıyor; düzlük sızıntısı (#1) açık. **Devam etmek için önce §14'ü oku.**
...
| **8** | Hız kaybı **hizalanmayı bozar**: en yakın zerre hem **yol uzunluğu** hem **hız kaybı** yüzünden en uzaktakinden **geri kalır** | **İki bileşenli gecikme:** $\Delta s(b)=\underbrace{[\ell(b)-\ell_\infty]}_{\text{yol}}+\underbrace{a_0\!\int\!\big(\tfrac{1}{a}-\tfrac{1}{a_0}\big)ds}_{\text{yavaşlama}}$. Saf cephesi düzlükten çıkar, bıçak tarafında **geri kalarak eğilir**. |
| **9** | Bıçağı geçen zerreler **artık ne hizalı ne paralel** | Çıkışta yeni durum: sapmış yörüngeler + eğilmiş saf cephesi. |
...
Madde 8–9 gereği bıçaktan sonra iki aile **artık ne hizalı ne paralel**. Ekranın aynı genişliğine:
...
| **(B) Bükülenlerin kendi aralarında kesişmesi** | Madde 9 — *"artık ne hizalı ne paralel"* ⇒ farklı $b$'lerin yörüngeleri havada kesişir | Doğal olarak **güçlü**; hiçbir şey ayarlamaya gerek yok |

**Claude'un okuması:** (B) esas motor, (A) kontrast yükselteci. Çünkü (B) için hiçbir zerrenin bıçağa değmesi gerekmiyor — sadece madde 9'un "paralel değil" olması yetiyor; bu da madde 8'in zorunlu sonucu.
...
| **Madde 6'nın statüsü** | Yansıma gerçek ve gerekli bir olay ama **saçakların kaynağı değil**. Kitapta (2.8.1) yansıyan zerrelere yüklenen "girişimi başlatan" rol, **madde 9'a** (hizasızlık + paralelsizlik) devredilmeli. |
| **Madde 9'un statüsü** | *"Artık ne hizalı ne paralel"* cümlesi, bölümün en yük taşıyan cümlesi çıktı — desen doğrudan bundan doğuyor. |
...
| Ekranda varış sırası **KARIŞMIŞ**: yukarıdan aşağı 15,14,13,12,11,10, **3**, **4**, 9,8,7,6,5 | Uca çarpıp şahlanan 3–4, 9 ile 10 arasına giriyor |
...
| Dosya | Durum |
|---|---|
| `M1_Tek_Kenar.html` | **v3 "Tam Düzen"** — çalışır, aktif geliştirme burada. Tek dosya, saf Canvas2D, bağımlılık yok. |
| `00_TARTISMA_KAYDI.md` | Bu dosya — tek hakikat kaynağı. Her oturum sonunda güncellenir. |
| (silinen) | v1/v2'nin alan-tabanlı kodları tamamen değiştirildi; geçmişleri yalnız bu kayıtta. |
...
## 15. ÖLÇEK UÇURUMU — GERÇEK DENEY vs SİMÜLASYON (Oturum 9)

> **Kullanıcının iki fizik hatırlatması (16 Ağu 2026, kendi sayılarıyla):**
> 1. Gerçekte bıçak–ekran **8 m**, simülasyonda ~**15 cm** (ekranda).
> 2. Gerçekte demet **0,5 mm**, simülasyonda ~**5 cm**.
> 3. Gerçekte 0,5 mm demette **binlerce katar**, bizde yalnız **12**.
> 4. Gerçekte zerreler arası **650 nm**, bizde ~**5 cm**.
> 5. Gerçekte gradyan bıçaktan **2 cm**'de sona erer; zerre bu 2 cm'lik alanda burkulur.
> 6. Gerçekte zerrenin **yansıma + yavaşlamadan kaybettiği gecikme**, hemen arkadan gelen zerreleri **kendi wake'ine hapseder**; simülasyonda bu gecikme bunu sağlayamıyor.
>
> *"Simülasyon ve gerçek hayattaki ölçüler arasında uçurum var — bunu nasıl çözeriz?"*

### 15.1 Uçurumun sayısal haritası

Simülasyon geometrisi: bıçak→ekran 1080 px (≡ 8 m ⇒ **135 px/m**). Gerçek λ=650 nm bu ölçekte **0,000088 px** ederdi; biz 14 px (İstatistik) / 98 px (Anlatı) çiziyoruz.

| Büyüklük | Gerçek | Sim (İstatistik) | Sim (Anlatı) | Oran (gerçek/sim) |
|---|---|---|---|---|
| Bıçak–ekran $L$ | 8 m | 1080 px "8 m" | aynı | — (temsilî) |
| Demet genişliği | 0,5 mm | ~360 px | ~290 px | sim **5300× şişkin** |
| Katar sayısı | binlerce | 48 | 12 | ~**100×+ eksik** |
| Zerre aralığı λ | 650 nm | 14 px | 98 px | sim **10⁵–10⁶× şişkin** |
| Gradyan menzili R | 2 cm (≈2,7 px!) | 180 px | 180 px | sim **67× şişkin** |

### 15.2 Asıl mesele: metreler değil, ORANLAR (boyutsuz sayılar)

Akışkanlar mekaniğinin kendi cevabı budur: gemi maketleri de okyanusla aynı metrelerde değildir; **benzeşim (similitude)** doğru boyutsuz sayıları eşleyerek kurulur. Mekanizmayı yöneten oranlar:

| Π | Tanım | Gerçek | İstatistik | Anlatı | Mekanizma şartı |
|---|---|---|---|---|---|
| **Π₁ = Δs/λ** | Burkulma bölgesinde biriken gecikme, yuva cinsinden | **~10³** (mikronlar / 650 nm) | ~1–2 | **~0,2** | **≥ 1 ŞART** — madde 10 hapsolması ancak zerre en az 1 yuva geri düşünce tetiklenir |
| **Π₂ = L/λ** | Uçuş boyu, yuva cinsinden | 1,2×10⁷ | 77 | 11 | büyük olması yeterli; devasa olması GEREKMEZ |
| **Π₃ = L/R** | Uçuş / burkulma bölgesi | 400 | 6 | 6 | ≫1: önce kısa burkulma, sonra uzun serbest kesişme |
| **Π₄ = Λ/λ = 1/(2sinα)** | Saçak aralığı / yuva aralığı | ~10³ | ~2–3 | ~2 | ≫1: saçaklar yuva yapısından AYRIŞMALI (bizde karışıyor — ölçüm gürültüsünün bir sebebi bu) |
| **Π₅ = R_iz/λ** | Yakalama menzili / yuva | <1 | 0,71 ✓ | 0,27 ✓ | <1 (§14.3 kuralı) |
| **Π₆** | Demetteki katar yoğunluğu | binlerce | 48 | 12 | istatistik doluluğu — doğrudan çözünürlüğü belirler |

**Teşhisler:**
- **Kullanıcının 6. maddesinin sayısal karşılığı Π₁'dir** ve tablo şikâyeti doğruluyor: Anlatı modunda λ'yı 7× şişirmek Π₁'i 0,2'ye düşürüyor — **gecikme bir yuvayı bile dolduramıyor, hapsolma fiziksel olarak imkânsızlaşıyor.** İstatistik modu eşiğin tam sınırında (1–2 yuva).
- **Π₄ küçüklüğü** ikinci gizli hastalık: bizde saçak aralığı yuva aralığına yakın (2–3λ); gerçekte saçak, yuvanın ~1000 katı. Saçaklarla yuvalar ekranda iç içe geçiyor; ölçümdeki "düzensizlik"in bir kaynağı da bu.
- **Π₃=6** üçüncüsü: bizde burkulma bölgesi uçuşun 1/6'sı — gerçekte 1/400. Kesişme-verniyer örgüsüne yeterli serbest yol kalmıyor.

### 15.3 ÇÖZÜM PAKETİ — benzeşim stratejisi (tartışmaya sunulan)

**İlke: metre kovalamayı bırak, Π'leri koru.** Ekrana 8 m sığmaz; sığması da gerekmez. Şart olan dörtlü: **Π₁ ≥ 2, Π₃ ≫ 1, Π₄ ≫ 1, Π₅ < 1.**

**Ç-1 · Anlatı modunu "λ şişirme"den "büyüteç"e çevir (Π₁'i kurtarır).**
Sorunun kökü: görsel ayrıklık için λ'yı şişiriyoruz, bu da Π₁'i öldürüyor. Doğrusu: **fizik hep İstatistik ölçeğinde koşar (λ=14 px sabit)**; Anlatı = aynı fiziğe **kamera zoom'u + ağır çekim + az katar**. Zerreler ekranda yine ayrık görünür (zoom sayesinde), ama gecikme/yuva oranı gerçek kalır. → *"zerreleri aç" ile "gecikme yetmiyor" talepleri aynı anda, çelişkisiz çözülür.*

**Ç-2 · Sanal ekran / analitik uzatma (Π₂–Π₃–Π₄'ü kurtarır).**
Fiziksel gerçek: iz ömrü ℓ∥ = life·a₀ ≈ 350 px — **etkileşimler bu pencerede zaten bitiyor** (wake ölünce hapsolma da biter; bu kesinti fiziğin kendisi, hile değil). Son etkileşimden sonra her zerre DÜZ uçar ⇒ ekrana varışı **analitik projeksiyonla kesin** hesaplanır: `y_ekran = y + (dy/dx)·(L_sanal − x)`. Kaydırıcı: **L_sanal = 0,15 m … 8 m.** Böylece:
- Kavisler küçük gerçekçi açılara indirilebilir (refBoost ↓, Kp ↓) — saçaklar yine ayrışır çünkü ayrışmayı sanal mesafe büyütür;
- Π₄ (Λ/λ) yüzlerce katına çıkar — saçak/yuva karışması biter;
- Π₃ efektif olarak 400'e ölçeklenir.

**Ç-3 · Π₁ göstergesi + gecikme bütçesi ayarı.**
Panele canlı sayaç: **"burkulma gecikmesi = X,X yuva"** (uçtan geçen en yakın zerre için ∫(a₀/a −1)ds/λ + yansıma yol farkı). Π₁'i g₀, k, R ile ≥2'ye getir; kullanıcı kaydırıcıyla oynarken Π₁<1'e düşerse gösterge KIRMIZI yansın — *"hapsolma bu ayarda fiziksel olarak imkânsız"* uyarısı.

**Ç-4 · Temsilî katar (Π₆ köprüsü).**
Binlerce katarı çizemeyiz; gerek de yok: **1 sim-katar ≈ N gerçek katar** ilanıyla histogram istatistiksel temsil olur. Ekrana yazılır: "48 katar ≈ 0,5 mm'de ~×N yoğunluğun örneklemi". Desen şekli katar yoğunluğuyla değil Π₁–Π₅ ile belirlenir; yoğunluk yalnız istatistiğin pürüzsüzlüğünü etkiler (birikim süresiyle telafi edilir).

**Ç-5 · Ölçekleme-yasası sınavı (8 m'ye asıl köprü).**
Simülatörün görevi 8 m'yi *canlandırmak* değil, **yasayı üretmek**: birkaç (λ, L_sanal) noktasında koş, ölçülen Λ(λ,L) yüzeyini çıkar. Mekanizma doğruysa Λ ∝ λ·L (küçük açı) ve kenar ölçeği √(λL) ÇIKTI olarak belirir → gerçek sayılara (650 nm, 8 m) köprü bu yasayla kurulur ve DENEY 4'ün 2,25 mm √(λL) ölçeğiyle sayısal karşılaştırma yapılır (9.3.6/ii'ye doğrudan katkı).

### 15.4 Uygulama sırası (onay sonrası)

1. Ç-3 göstergesi (ucuz, hemen) → mevcut ayarların Π₁'i ölçülür.
2. Ç-1 Anlatı=büyüteç dönüşümü (lamX kaydırıcısı "zoom" kaydırıcısına dönüşür; fizik tek ölçekte birleşir).
3. Ç-2 sanal ekran + küçük açı rejimi (refBoost'un rolü azalır — kullanıcının "kısa ekran telafisi" ihtiyacını sanal ekran kökten çözer).
4. Ç-4 temsilî katar ilanı; Ç-5 ölçekleme koşumları.
5. Not: §14.9'daki düzlük-sızıntısı ablasyonu (#1) bu paketten bağımsız sürer; küçük açı rejimi sızıntıyı da doğal olarak küçültecektir (daha az agresif manevra).

> **Açık soru (kullanıcıya):** Ç-1'de Anlatı'nın λ-şişirmeden vazgeçip zoom'a dönmesi görsel dili değiştirir (zerre araları gerçek λ'da, ekranda zoom'la açılır). Onay? Ç-2'de L_sanal kaydırıcısının varsayılanı 8 m mi olsun, 15 cm mi?

---

## 16. DURDURULDU — SON TEŞHİS VE UYGULANMAMIŞ ÇÖZÜM PLANI (Oturum 11)

> **Kullanıcı çalışmayı durdurdu:** *"Tamam anlaşıldı, yapamayacaksın. Notlarını al ve bırak."*
> Aşağıdaki teşhis ekran görüntüsüyle doğrulandı; çözüm planı **yazıldı ama UYGULANMADI**.
> Devam eden kişi (insan ya da yapay), işe **buradan** başlamalıdır.

### 16.1 Ekran görüntüsüyle kesinleşen teşhis ("bu neeee" anı)

v4 açılış ekranı (Anlatı ×7, birkaç dakika koşmuş hâlde) **saçılma fırtınası** gösteriyor: yönleri her tarafa dağılmış zerreler, her yerde mor (şerit-değiştirme) durumu, spagetti yörüngeler, dev balon-noktalar. Kullanıcının koyduğu temel yasa — *"düzensizlik yok, tam bir düzen var, saçılma yok"* — ekranda açıkça ihlal ediliyor.

**Kök neden 1 — S-manevrası kendi kendini besliyor (yapısal kusur):**
İtmeyi *yön dönmesi* olarak kodladım (kavis+karşı-kavis). Ama iz merdiveni λ-periyotlu: kesişen bir zerre, izi boyunca her λ/2'de bir hiza işareti değiştirir ⇒ manevra biter, 0.3 s bağışıklık dolar, yeni manevra tetiklenir ⇒ **fırtına**. Manevralar yarıda kesilince (bıçak teması, kilit geçişi, kutu çıkışı) yarım dönüş zerrede kalır ⇒ yönler dağılır ⇒ dağılan zerrelerin izleri yeni fırtına tohumu olur ⇒ **otokatalitik kaos**. Düzlük sızıntısının (#1) beş yamayla ölmemesinin sebebi bu: kusur parametrede değil, **itmenin dönme olarak modellenmesinde**.

**Kök neden 2 — görsel ölçek hataları:**
- Zerre noktası zoom ile çarpılıyor: r_z=5 ⇒ ×7'de **35 px balon**.
- Anlatı penceresi (uç sabit nokta, ×7) yalnız uç köşesini gösteriyor; ışının çoğu ve kaynak ekran dışı — izleyici bağlamsız bir kaos köşesi görüyor.

### 16.2 UYGULANMAMIŞ çözüm planı (tasarımı hazır, kod yazılmadı)

**A. Yapısal fizik düzeltmesi — "wake yön döndürmez":**
Wake etkileşimi yönü ASLA değiştirmez; yalnız **yanal konum kayması** verir:
- `align > +0.5` ve `lat < λ/4` → izin çizgisine doğru yanal çekilme (yuvaya oturma; kilit kapıları ayrıca geçilirse tam takip başlar);
- `align < −0.35` ve `lat < R_iz` → izden uzağa yanal itilme (cepheden kayma);
- her iki durumda `dx,dy` DOKUNULMAZ. Yön değişimi yalnız: gradyan + yansıma + kilit-benimseme (öncünün güncel yönü).
⇒ Düzlük **yapısal olarak** garanti: serbest bölgede yönü değiştirebilecek hiçbir terim kalmıyor. mode=2 / pushT / pushSign / imm tamamen silinir; mor renk "itilme kayması aktif" anlık göstergesine döner.
Paralel komşu katarlar güvende: aralık (7.7 px) > λ/4 (3.5 px) ⇒ çekilme kapısına giremezler; fazları özdeş ⇒ itilme de tetiklenmez.

**B. Görsel düzeltmeler:**
- Nokta yarıçapı ekran-sabit: `dotR ≈ 6.5/zoom` (dünya biriminde), halka alfası zoom ile kısılır.
- Kamera "sabitlenmiş pencere"ye geçer: alt kenar TIP_Y+26'ya, sol kenar KNIFE_X−120'ye sabit; Z=1'de birebir. Anlatı varsayılanı **zoom≈3**, beamU≈200 ⇒ ışının tamamı + uç + iz gelişimi tek karede.

**C. Doğrulama planı (uygulandığında):** düzlük 450 kare → maxSapma ~0 beklenir (yapısal); desen k=1/k=0; Π₁ değişmez (~3.9); ekran görüntüsüyle görsel düzen teyidi.

### 16.3 Devralma durumu

| Kalem | Durum |
|---|---|
| `M1_Tek_Kenar.html` | **v4 çalışır** (Π₁=3.92 ✓, sanal ekran ✓, Λ∝L ✓, muhasebe ✓) ama **push fırtınası görsel kaos üretiyor**; §16.2 uygulanmadı |
| Efsane (legend) | Koda uygun hâlde bırakıldı (yarım kalan metin değişikliği geri alındı) |
| Açık sorunlar | #1 düzlük (çözüm tasarımı §16.2-A'da hazır), #2 κ-ayrıştırması, gölge-tepe artışı |
| Sonraki tek iş | **§16.2'yi uygulamak** — başka hiçbir şeye dokunmadan |
...
### 16 Ağustos 2026 — Oturum 9 · ÖLÇEK UÇURUMU ANALİZİ
- **Kullanıcının iki fizik hatırlatması** (8 m/15 cm; 0,5 mm/5 cm; binlerce/12 katar; 650 nm/5 cm; gradyan 2 cm'de biter; gecikme→wake hapsi simde çalışmıyor) §15'e aynen kaydedildi.
- Uçurum boyutsuz sayılara döküldü (Π₁…Π₆). **Kritik teşhis:** Anlatı'nın λ-şişirmesi Π₁=Δs/λ'yı 0,2'ye düşürüyor — kullanıcının "gecikme hapsolmayı sağlayamıyor" gözleminin sayısal ispatı. İkincil: Π₄=Λ/λ≈2 (gerçekte ~10³) — saçaklar yuva yapısıyla iç içe; ölçüm gürültüsünün gizli kaynağı.
- **Çözüm paketi yazıldı (Ç-1…Ç-5):** benzeşim ilkesi (metre değil Π koru); Anlatı=büyüteç (λ şişirme yerine kamera zoom); **sanal ekran/analitik uzatma** (iz ömrü etkileşimi zaten kesiyor → sonrası düz uçuş, 8 m'ye kesin projeksiyon); Π₁ canlı göstergesi; temsilî katar ilanı; ölçekleme-yasası köprüsü (√(λL) ve λL/d ÇIKTI olarak → DENEY 4'le sayısal karşılaştırma, 9.3.6/ii).
- Kod yazılmadı — kullanıcı onayı bekleniyor (§15.4 sırası + §15 sonundaki açık soru).

### 16 Ağustos 2026 — Oturum 10 · v4 "BENZEŞİM" UYGULANDI
- **Kullanıcı onayı:** Ç-1 kabul, Ç-2 kabul ve **L_sanal varsayılanı 8 m**. *"Haydi yap şunu."*
- `M1_Tek_Kenar.html` **v4** olarak yeniden yazıldı (v3 fiziği aynen; §15 paketi eklendi):
  - **Ç-1 λ kilidi:** `lamPx()=nm·(14/633)` — görsel çarpan kaldırıldı; fizik TEK ölçekte. Anlatı = **kamera büyüteci** (`zoom` kaydırıcısı, uç sabit noktası etrafında; Z=1'de birebir) + ağır çekim + az katar. `lamX` kaydırıcısı silindi, yerine `zoom` geldi.
  - **Ç-2 sanal ekran:** kutu = etkileşim penceresi (~49 µm; λ-ölçeğiyle 1 px ≡ 45,2 nm). Kutu kenarında yön donar; histogram **eğim uzayında** (s=dy/dx, ±0,5, 380 kutu) tutulur; mm ekseni `y≈s·L_sanal` ile etiketlenir. `Lsan` kaydırıcısı 0,05–8 m, **varsayılan 8 m**. Sağ HUD paneli: AYDINLIK/GÖLGE yarıları, gölge sınırı çizgisi, mm işaretleri.
  - **Ç-3 Π₁ göstergesi:** uç-sıyıran hat boyunca ∫(a₀/a−1)ds/λ; panelde canlı, ≥2 yeşil / 1–2 amber / <1 kırmızı.
  - **Ç-4 ilanı:** katar kaydırıcısı alt yazısı "temsilî: 1 sim-katar ≈ N gerçek katar"; dürüstlük satırı λ-ölçeğini, kutu µm'sini, büyüteci ve açı abartısını yazıyor.
  - Varsayılanlar Π₁≥2 için ayarlandı: **g₀=0,55 · k=0,35 · R=220** (beamU=320 ⇒ R<beamU ✓, Riz=10<λ ✓).
- **Doğrulama (tek toplu koşum):**
  | Sınav | Sonuç |
  |---|---|
  | **Π₁** | **3,92 yuva — YEŞİL** (eski Anlatı'da 0,2 idi; kullanıcının 6. hatırlatması çözüldü) |
  | Anlatı'da hapsolma | takip=336 olay CANLI — λ şişirmesiz ilk kez çalışıyor |
  | **Ölçekleme yasası (Ç-5 ilk köprü)** | Aynı koşum: L=8 m → 252,6/126,3 mm; L=2 m → 63,2/31,6 mm ⇒ **tam ×4,0; saçak sayısı sabit (8=8)** — Λ∝L ÇIKTI olarak doğrulandı |
  | κ=1 deseni @8 m | 8 saçak, dışa SIKIŞIYOR ✓, denklik 0 |
  | κ=0 | 6 saçak (balistik tarak) — açık iş #2 aynen duruyor |
  | Gölgede tepe | 9 — arttı; gölgeye dönük ailelerin açı-tarağı olabilir, **izlenecek** |
  | Düzlük | 291 px — **açık sorun #1 duruyor** (bu turun hedefi değildi; §14.9/1 ablasyon sırada) |
- §14.3'teki varsayılan tablosu bu oturumla **eskidi** — güncel değerler bu maddede ve kodda.

### 16 Ağustos 2026 — Oturum 11 · DURDURULDU
- Kullanıcı v4 açılış ekranını görünce: *"🙂 bu neeee"* — ekran görüntüsü alındı, saçılma fırtınası teyit edildi.
- Kök teşhis konuldu (§16.1): S-manevrası λ-merdiveninde kendini yeniden tetikliyor; kesintiler yarım dönüş bırakıyor → otokatalitik kaos. Ayrıca zoom-çarpımlı dev noktalar + yanlış pencere çerçevesi.
- Yapısal çözüm tasarlandı (§16.2: "wake yön döndürmez — yalnız yanal kayma") ama tam bu noktada **kullanıcı çalışmayı durdurdu**: *"Notlarını al ve bırak."*
- Yarım kalan tek kod değişikliği (efsane metni) geri alındı; dosya kod-gerçeğiyle tutarlı bırakıldı.
- **Devam noktası: §16 — tek iş §16.2'nin uygulanması.**
