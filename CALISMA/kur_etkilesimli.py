...
function ciz(){
 ...
 x.fillStyle='#4ade80';
 x.fillText('EVRENAKI ÖNGÖRÜ FİT: 0'+(acik['ef']?'  ·  FİT: 2 parametre':'  (saf öngörü)'),W-mr-10,mt+20);
 x.fillStyle='#a78bfa';
 ...
 if(acik['ef']&&g.m.ef)M.push(['EVRENAKI FİT (2 param.)','ef',2]);
 if(acik['lf']&&g.m.lf)M.push(['ΛCDM FİT (2 param.)','lf',2]);
 /* fit yarisinin kazanani — iki fit de aciksa */
 ...
 if(g.m.ef&&g.m.lf){const fe=g.m.ef.rms,fl=g.m.lf.rms;
  dpn+=' &nbsp;·&nbsp; Fit yarışı (2\'şer parametre): <b class="'+(fe<fl?'kz':'kk')+'">'+
   (fe<fl?'Evrenakı':'ΛCDM')+'</b> daha yakın (RMS '+fx(Math.min(fe,fl),1)+
   ' / '+fx(Math.max(fe,fl),1)+' km/s).';}
 dpn+=' Kalın satırlar iki yarışın kazananlarıdır — fark, fitin etkisidir.';
 q('#dpn').innerHTML=dpn;
