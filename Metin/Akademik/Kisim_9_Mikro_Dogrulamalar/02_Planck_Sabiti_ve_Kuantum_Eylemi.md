...
1. **Özdeşlik yolu:** $h=\delta\tau$ içine $\delta$'nın açık ifadesi konduğunda (dönme terimi dahil; $k_a v_{cev}^2=c_0^2$) $h = 4\tau m_z^2c^2/m_e$ çıkar; ölçülen $h$ bu bağıntıdan $\tau = h/\delta \approx 7{,}7$ ps ister.
...
>
> Ayrıştırmanın kendisi yine de boş değildir: $\delta$ artık $m_z$'den tam hesaplanır ve ayarlanabilir hiçbir şey içermez ($\delta=4m_z^2c^2/m_e$), dolayısıyla $h$'ın "kesikliliği" bir doğa gizemi olmaktan çıkıp **tek vuruşun aktardığı enerji × alıcının kopma penceresi** biçiminde mekanik bir okumaya kavuşur. Kazanılan şey bir sayı değil, bir **yorumdur**.
>
...
|---|---|---|---|
| **Taşınan** (Zerre başına) | $E_{Zerre}=\tfrac12 m_z(c_0^2+k_a v_{cev}^2)=m_zc^2$ | **8,25 eV** (öteleme 4,12 + dönme 4,12) | frekanstan bağımsız, evrensel |
...
Bugünkü durum bir **iç tutarlılık kilididir**: $\delta\tau$ çarpımı fotoelektrik eğimden sabitlenir; Compton, karacisim ve Casimir aynı çarpımı bağımsız ailelerde geri okur; ama $\delta$ ile $\tau$ ayrı ayrı ölçülmemiştir (rozet [S], Ek C). $\delta$'nın bileşenleri kuruludur — $k_a=1/2$ ve $v_{cev}=\sqrt2\,c_0$ türetilmiş, $m_z$ ise teorinin ölçülen girdi parametresidir; dolayısıyla $\delta=\eta\,m_zc^2$ kapalı hesaplanır ve $\tau=h/\delta$ zorunlu çıkar.
