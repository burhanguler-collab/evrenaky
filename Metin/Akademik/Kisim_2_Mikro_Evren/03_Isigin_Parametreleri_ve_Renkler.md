...
Işığın sabit bir hızı ($c_0$) olduğunu varsaydığımızda (bölgesel Evrenakı yoğunluğu sabitken), peş peşe gelen zerrelerin arasındaki mesafe ($\lambda$) daraldıkça, doğal olarak hedefe bir saniye içinde çarpacak olan zerre sayısı (frekans) da artacaktır. İşte bu ters orantı $c_0 = \lambda \cdot \nu$ denkleminin Evrenakı'daki yegâne gerçekliğidir.
