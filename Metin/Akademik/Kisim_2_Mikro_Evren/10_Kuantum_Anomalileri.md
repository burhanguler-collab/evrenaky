...
Standart yorumda belirsizlik ilkesi, doğanın dokusuna içkin bir bulanıklık olarak sunulur: konum ve momentum aynı anda keyfî kesinlikle tanımlı olamaz. Dürüst kayıt: modern literatür bu ilkeyi iki ayrı katmanda formüle eder. **(1) Hazırlık belirsizliği** (Kennard–Robertson eşitsizliği; Kennard, 1927; Robertson, 1929): aynı biçimde hazırlanmış bir topluluk üzerinde konum ve momentum ölçümlerinin *istatistiksel dağılımları* $\sigma_x \sigma_p \geq \hbar/2$ bağıntısına uyar — bu ifade tek ölçümün verdiği hasar hakkında değil, dağılımlar hakkındadır. **(2) Ölçüm-bozması:** tek bir ölçümün sistemi ne kadar bozduğu; Heisenberg'in mikroskop argümanının (Heisenberg, 1927) modern hâli, literatürde ayrı (Ozawa tipi) ayrıştırmalarla incelenir (Ozawa, 2003).
...
```mermaid
graph TD
    A[Zerre Sağanağı Akışkanda İlerliyor] --> B{Zerre Nereye Çarptı?}
    
    B -->|Açık Rampaya<br>Senkronize Girdi| C[Süzülme / Kırılma<br>Bölüm 2.5]
    B -->|Kapalı Rampaya<br>Çarptı| D[Elastik Sekme<br>Yansıma - Bölüm 2.5]
    B -->|Sert Yüzeye<br>Eşzamanlı Çarptı| E[Kümülatif Sarsıntı<br>Fotoelektrik Etki]
    
    B -->|Dar Yarıktan Geçti| F[Ardıl Vorteks Bıraktı]
    F -->|Sensör Yok| G[Vorteksler Düzeni Korur<br>Girişim Deseni]
    ...
    classDef root fill:#0a0a1a,stroke:#ffb020,stroke-width:2px,color:#fff;
    
    class A root;
    class B root;
    class C,D,E,F,G,I,J mekanik;
    class H anomaly;
```
