...
### 7. Sürtünmesizlik ve Sürüklenme (Entrainment) İlkesi

Gök cisimlerinin akışkan ortamda kinetik enerji kaybetmeden hareket etmesi, Evrenakı viskozitesinin sıfıra yakın ($\mu \approx 0$) olması ve **"Sürüklenme Zarfı"** (boundary layer / entrainment) mekanizmasıyla açıklanır. Gezegenler, etraflarındaki lokal Evrenakı tabakasını kendileriyle birlikte sürükler; zarf içindeki akışkan ile cisim arasındaki bağıl hız sıfırdır:
...
Galaksilerin ve yıldızların dönüşleri, Evrenakı ortamında üç boyutlu makro-girdaplar oluşturur. Bu girdaplar, akışkanlar mekaniğinin sıfır viskozite limitindeki Euler ve Navier–Stokes denklemleriyle modellenir (Euler, 1757; Navier, 1823; Stokes, 1845b); lokal vortisiti $\vec{\Omega} = \frac{1}{2}(\nabla \times \vec{v})$ ile tanımlanır. Evrenakı girdapları; galaksilerin dış kollarındaki dönüş hızı anomalilerini (rotasyon eğrilerini) (Rubin & Ford, 1970), ek bir karanlık madde varsayımına ihtiyaç duymaksızın, akışkanın rotasyonel ve yanal basınç kuvvetleriyle açıklamayı önerir.
...
| 14 | $\eta_E$ | Evrenakı viskozitesi (3.10'da $\eta_z$ olarak geçer; aynı büyüklüktür) | **F** | $0^+$ (süper-akışkan) | Gaia / pulsar katalog korelasyonları (Gaia Collaboration, 2021) (3.1) + Satürn bending-wave sönüm testi (3.10.5) | 3.1, 3.7.2, 3.10 |
