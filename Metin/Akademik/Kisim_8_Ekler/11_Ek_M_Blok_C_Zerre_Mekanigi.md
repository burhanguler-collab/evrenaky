...
**Kullanıldığı bölümler:** 2.2.2–2.2.3, 2.4.2 (Yön Kuralı kutusundaki $(c^2+k_a v_{cev}^2)$ atfı), Postülat 4 (1.3), 2.6.5, 7.4.
...
4. Her Zerre, öteleme enerjisine ek olarak evrensel çevresel hızı $v_{cev}$ ile dönme enerjisi taşır; atalet katsayısı $k_a$'dır (homojen katı küre için $k_a = 2/5$).*

*\*Notasyon notu (Anayasa S-1): atalet katsayısı gövde metninde (2.2.2) $k$ olarak geçer; $k$ sembolü kitap genelinde ρ–P eşlik oranına ayrıldığından katalogda **$k_a$** kullanılır.*
...
$$E_{Zerre} = \tfrac12 m_z c^2 + \tfrac12 k_a\, m_z v_{cev}^2 = \tfrac12 m_z\left(c^2 + k_a\, v_{cev}^2\right)$$
...
### Sonuç
$$\boxed{\Phi + E_{ke} = N \cdot \eta \cdot \tfrac{1}{2} m_z \left(c^2 + k_a\, v_{cev}^2\right)\,, \qquad \eta \approx \frac{4\,m_z}{m_e} \approx 6{,}5\times10^{-5}}$$

**Sayısal zincir:** $\tfrac12 m_z c^2 \approx 4{,}1$ eV (tek Zerre'nin öteleme enerjisi) → tek vuruşta aktarılan pay $\eta \cdot \tfrac12 m_z c^2 \approx 2{,}8\times10^{-4}$ eV → tipik sökme işi ($\Phi + E_{ke} \approx 4$ eV) için $N \approx 1{,}5\times10^4$ vuruş (dönme terimi bu sayıyı $v_{cev}$'in büyüklüğü oranında düşürür) → morötesi ritminde ($\nu \sim 10^{15}$ Hz) birikim süresi $N/\nu \approx 10^{-11}$ s ≈ **10 pikosaniye** — ölçülen nanosaniye-altı "gecikmesiz" emisyonla (Lawrence & Beams, 1927) uyumlu.
...
### Açık Uçlar
- $v_{cev}$'in bağımsız sayısal tespiti (7.4); dönme teriminin muhasebedeki payı buna bağlıdır.
...
$$\delta = \eta \cdot \tfrac{1}{2} m_z \left(c^2 + k_a\, v_{cev}^2\right)$$
...
Standart fiziğin "foton" adını verdiği ölçüm birimine teoride karşılık gelen fiziksel gerçeklik budur; enerji yalnızca tam paket kuantumlarıyla ($N_p \cdot h\nu$) teslim edilir.
...
### Geçerlilik Sınırı
- Rozet **[S]**: gözlemle sabitlenen şey **çarpım** $(\delta\tau)$'dur; bileşenler $\delta$ ve $\tau$ ayrı ayrı serbest parametrelerdir (Ek C). $h$'nin ölçülmüş değeri çarpıma tek bir kısıt koyar, iki bilinmeyeni tek tek çözmez.
...
### Açık Uçlar
- $\delta$ ve $\tau$'nun bağımsız tespiti (7.4); manüskriptin açık işidir.
...
- **Klasik elektron yarıçapı** $r_e \approx 2{,}8\times10^{-15}$ m (Tiesinga ve ark., 2021) ile kıyas: $r_z/r_e \approx 8\times10^{-4}$ — yarıçapça yaklaşık **binde bir**. (Kıyasın hangi yarıçapla yapıldığı zorunlu olarak belirtilir: Evrenakı Teorisi'nde elektron noktasal değil, geniş hacim süpüren bir girdaptır; bkz. 2.1.)
...
### Geçerlilik Sınırı
Sonuç, iki Postülat 4 girdisinin ($m_z$, $\rho_n$) aritmetik sonucudur; rozet **[T]** bu türetimin kendi içindeki zorunluluğunu işaretler, girdilerin statüsünü ($m_z$: fotoelektrik veriyle sabitlenmiş, bkz. M-10) devralmaz. Küre idealizasyonu $O(1)$ geometri düzeltmesine açıktır.

### Açık Uçlar
- Zerre'nin gerçek girdap geometrisinin (küre yerine zarf/torus benzeri yapının) $r_z$'ye düzeltmesi.
...
### Varsayımlar
1. Işığın patinajı, Evrenakı'nın $P/\rho$ oranının düştüğü bölgelerin olayıdır ($c = \sqrt{P/\rho}$, M-1); nötrino tutunma mekanizmasına bağımlı değildir ve oyalanmadan geçer (2.1, 2.4.4).
...
**Kullanıldığı bölümler:** 2.10.1, 2.9.2.1 (Malus geçit mekaniği), Ek A.3, Ek B.4 (vakum kararlılığı), M-4, M-5, 6.0, 7.4, 7.5.

### Varsayımlar
1. M-5'in sonucu: kohezyon kanalının elastik sinyal hızı $v_m = \sqrt{\Sigma/\rho_0} = c\sqrt{\Sigma/P_0}$; bu kanal enerji/madde taşımaz, ortam topografyasının (gradyan deseninin) ayar sinyalidir.
...
3. Bell-tipi "etki hızı" deneyleri, kanatlar arası koordinasyon hızına gözlemsel alt sınır koyar: $v > 10^4\,c$ (Salart ve ark., 2008).
...
$$v_{kav} \approx \sqrt{2}\,v_m > 1{,}4\times10^4\,c$$
...
Ölçülmedikçe her yeni deney alt sınırı yükseltir — iki sonuç da bilgi vericidir: dolanıklık, madde doğumu ve vakum kararlılığı (Ek B.4) tek $\Sigma$ üzerinde kenetlenir.

### Sonuç
$$\boxed{\frac{\Sigma}{P_0} = \left(\frac{v_m}{c}\right)^2 > 10^8\,, \qquad v_{kav} \approx \sqrt{2}\,v_m > 1{,}4\times10^4\,c\,, \qquad \Sigma = P_0\left(\frac{v_m}{c}\right)^2}$$
...
- **GW170817 notu:** kütleçekim dalgası* hızının $10^{-15}$ hassasiyetle $c$'ye eşitlenmesi (Abbott ve ark., 2017) **sıkışma kanalını** bağlar; kohezyon kanalını bağlamaz. Teoride "kütleçekim dalgası" denen basınç salınımları basınç kanalının olayıdır ve $c$ ile yayılır; kütle-itim alanının statik topografya kurulumu ise kohezyon kanalını kullanır — iki-kanal ayrımının beklediği tablo tam budur (2.10.1).
