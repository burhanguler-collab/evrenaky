...
*Notasyon notu:* Gövde metni (6.1) tarihsel Doppler gösterimine sadık kalarak $f_0, f_s, f_{obs}$ yazar; bu katalogda Anayasa gereği (R-2, S-15) frekans $\nu$ ile gösterilir: $\nu_0$ (kaynağın öz/durgun frekansı), $\nu_s$ (hareketli kaynağın ateşleme frekansı), $\nu_{obs}$ (alıcının ölçtüğü frekans). Buradaki frekans, **tek bir katarın içindeki** ardışık Zerre ritmidir (şiddet/katar sayısı değildir; bkz. D-21 tanımı, 2.2.3).

### Varsayımlar
1. Işık, $\nu$ frekansıyla (saniyedeki Zerre sayısıyla) ateşlenen bir **Zerre Katarı**dır (mermi dizisi); yerel ortamda çizgisel Zerre hızı $c$'dir (M-1).
...
3. Ateşleme dişlisi (atomik geçişler), akışkana kavrama mekanizmasıyla çalışır ve $c$ hızına tabidir: atom içinde bir referans noktasından diğerine $c$ ile gidip dönen iç sinyal, mekanik bir **Zerre-Saati** (ışık saati) oluşturur.
...
$$ \lambda = c\,T_s + v\,T_s = \frac{c + v}{\nu_s} $$
(Yaklaşan kaynak için $\lambda = (c - v)/\nu_s$.)

**2. Zamansal kol — mekanik saatin yavaşlaması.** Atom akışkan içinde $v$ hızıyla ilerlerken, $c$-limitli iç sinyal düz yol yerine **çapraz (üçgen) yol** izlemek zorundadır. Durgun gidiş-dönüş süresi $T_0$, hareketli süre $T$ ise Pisagor'dan:
$$ (c\,T)^2 = (v\,T)^2 + (c\,T_0)^2 \;\Longrightarrow\; T = \frac{T_0}{\sqrt{1 - v^2/c^2}} $$
...
$$ \nu_{obs} = \nu_0 \left( \frac{1 \pm u/c}{1 \pm v/c} \right) \frac{\sqrt{1 - v^2/c^2}}{\sqrt{1 - u^2/c^2}} $$
...
- Kaynak ve alıcı **aynı yoğunluktadır** ($c$ ortak). Yoğunluklar farklıysa M-20'nin birleşik formülü geçerlidir.
- Kinematik katmanda SR'den sapma öngörülmez; ayrışma dinamik katmandadır (değişken $c$; M-20).
...
*Terminoloji notu:* "Kütleçekimsel kızıla kayma" (gravitational redshift) yerleşik **gözlem adı** olarak korunur; mekanizma teoride kütleçekim değil, **kütle-itim gradyanının** ördüğü basınç kuyusu / yoğunluk profilidir ($c = \sqrt{P/\rho}$ üzerinden).

### Varsayımlar
1. $c = \sqrt{P/\rho}$ yereldir ve değişkendir (M-1, Postülat 4): kütle çevresindeki basınç kuyusunda (yüksek yoğunluk bölgesinde) yayılma hızı $c_{loc}$ düşüktür (Yön Kuralı, 2.4.2 / Ek B.3, M-8: $\delta c/c = \frac{1-k}{2}\,\delta P/P_0 < 0$).
...
4. **Ölçek ayrımı (M-42):** "Yerel ışık hızı" tek büyüklük değildir. **Madde ölçeği** $\Lambda \equiv 1-\Phi/c^2$ cetvelleri, saatleri ve atomik geçiş frekanslarını yönetir; **yayılma hızı** ise $c_{loc} = c\Lambda^2$'dir. Bu girdinin *üretim* ve *ölçüm* adımları $\Lambda$ ile, *yayılım* (Zerre Aralığı) adımı $c_{loc}$ ile yazılır.
...
### Adımlar
Hareket olmasın ($u = v = 0$, $\gamma = 1$); kaynak basınç kuyusunun dibinde (yüksek yoğunluk, küçük $\Lambda_{kaynak}$), alıcı serbest uzayda (düşük yoğunluk, büyük $\Lambda_{alıcı}$) olsun.
...
$$ \lambda(r) = \frac{c_{loc}(r)}{\nu_{emit}}\,, \qquad c_{loc} = c\,\Lambda^2 $$
...
Birinci mertebede $\Lambda = 1-\Phi/c^2$ olduğundan bu oran ölçülen genliği doğrudan verir: $\delta\nu/\nu = -\Phi/c^2$ (Pound–Rebka 1960; GPS: Ashby 2003).
...
- **Saat/yayılım ayrımı zorunludur (M-42):** üretim ve ölçüm adımlarında $\Lambda$ yerine yayılma hızı $c_{loc} = c\Lambda^2$ kullanılırsa kızıla kayma genliği **iki kat** büyük çıkar ($-2\Phi/c^2$) ve Pound–Rebka/GPS ile çelişir.
...
3. **Ölçek ayrımı (M-42):** $\Lambda \equiv 1-\Phi/c^2$ **madde ölçeğidir** — cetvelleri ($\ell_{loc}\propto\Lambda$), saatleri ve atomik geçiş frekanslarını yönetir. Zerre'nin **yayılma hızı** ise ayrı bir büyüklüktür: $c_{loc} = c\Lambda^2$. "Yerel ışık hızı" ifadesi bu ikisini karıştırmamak için bu girdide kullanılmaz.
...
Yoğunluk artıp $\Lambda$ düşerse aynı iç çevrim daha uzun sürer → tik yavaşlar.
...
### Sonuç
$$\boxed{\nu_{tik} \propto \frac{\Lambda}{\gamma}\,, \qquad \Lambda \equiv 1 - \frac{\Phi}{c^2}\,, \qquad \gamma = \frac{1}{\sqrt{1 - v^2/c^2}}}$$
...
- **Bağıntıda $\Lambda$ durur, $c_{loc}$ durmaz.** Bu ayrım olmadan — yani tik hızı yayılma hızıyla ($c_{loc} = c\Lambda^2$) orantılı sayılırsa — kızıla kayma genliği **iki kat** büyük çıkar ($-2\Phi/c^2$ yerine doğrusu $-\Phi/c^2$) ve Pound–Rebka (1960) ile GPS (Ashby, 2003) verisiyle çelişir. Bükülme ve Shapiro gecikmesi ise tersine $c_{loc}$ ile yazılmak zorundadır; $\Lambda$ ile yazılırsa bükülme yarıya iner (M-42).
