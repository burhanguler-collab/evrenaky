...
Alt sınır yerine gerçek değeri, teorinin kendi mekanizmasından sabitlemek mümkündür. Kütleye yaklaşıldıkça ortamın durumu değişir; bu değişimde yoğunluğun basınca eşlik oranını $\delta\rho/\rho_0=k\,\delta P/P_0$ ile parametrize edelim. **Bu, maddenin doğurduğu *deplasman* sürecidir** ve ortamın adiyabatik dalga tepkisiyle karıştırılmamalıdır (ikisi ayrı kısmi türevlerdir — Ek M-44). Deplasman süreci için $k=0$'dır: **basınç düşer, ortalama yoğunluk korunur.** Bu değer serbest değildir; Ek M-15'in G1 (Korunum) aksiyomu ($\bar\rho_m=\rho_0$) ile Ek M-30'un Varsayım 1'i ("gradyanlarda asıl değişen basınçtır") bağımsız olarak aynı sonucu verir; M-44'ün iki değişkenli hâl denklemi bunu kutulu sonuca çevirir. Kavrama Yasası'ndan, yerel ışık hızındaki — ve Postülat 3 gereği yerel saat hızındaki — oransal kayma (kuyuda $\delta P<0 \Rightarrow \delta c<0$; izleyen eşitlikler M-8'deki gibi **genlikler** üzerinden okunur):

$$\frac{\delta c}{c_0}=\frac{1}{2}\left(\frac{\delta P}{P_0}-\frac{\delta\rho}{\rho_0}\right)=\frac{1-k}{2}\cdot\frac{\Delta P_{yüzey}}{P_0}$$

Bu kaymanın genliğini veren gözlem, ölçek yapısının (Ek M-42) belirlediği çarpanla okunur. Madde ölçeği $\Lambda=1-\Phi/c_0^2$ *(birinci mertebede; tam biçim $\Lambda=e^{-\Phi/c_0^2}$ — **Ek M-55**)* ile saat $f\propto\Lambda$, yayılma hızı $c_{loc}\propto\Lambda^2$ ölçeklendiğinden **saat kayması ile yayılma hızı kayması eşit değildir:** $\delta c/c_0 = 2\,\delta f/f$, yani $\delta c/c_0 = -2\Phi/c_0^2$ ($\Phi/c_0^2\approx7\times10^{-10}$; $\Phi$: yüzey kütleçekim potansiyeli). Bu 2 çarpanı ışık bükülmesi ölçümünden ($1{,}751''$) sabitlenmiştir; $\delta c/c_0=\delta f/f$ okuması bükülmeyi gözlenenin yarısı verir ve dışlanmıştır (M-42). Yüzeydeki basınç açığı ise gradyanın **dış bölge** integralinden gelir: $\Delta P_{yüzey}=\int_R^\infty \rho_n g\,dr=\rho_n\Phi$. (Ek B.2'nin *iç bölge* integrali ayrı büyüklüktür — merkez–yüzey ek düşüşü, bunun yarısı kadardır.) İkisi eşitlenince $\Phi$ sadeleşir ve arka plan basıncı, kütleden bağımsız **evrensel** bir değere sabitlenir:

$$\frac{1-k}{2}\cdot\frac{\rho_n\Phi}{P_0}=\frac{2\Phi}{c_0^2}\;\Longrightarrow\;\boxed{P_0=\frac{1-k}{4}\,\rho_n c_0^2\;\xrightarrow{\;k=0\;}\;6{,}07\times10^{33}\ \text{Pa},\qquad \rho_0=\frac{P_0}{c_0^2}=\frac{\rho_n}{4}\approx6{,}8\times10^{16}\ \text{kg/m}^3}$$
...
2. **Zayıf-alan kontrolü.** Dünya'nın merkez–yüzey basınç düşüşü ($\Delta P\approx0{,}83\times10^{25}$ Pa; Ek B.2), $P_0$'ın yanında $\sim10^{-9}$'luk bir pürüzdür. Kütle-itim etkilerinin gözlenen küçüklüğü (saat kaymalarının $10^{-9}$–$10^{-10}$ mertebesi) doğrudan bu orandır: gezegenler, okyanusun milyarda-birlik dalgacıklarıdır.
...
Postülat 1'deki "ağırlıksız ortam" ifadesi bu ekte kanıt statüsü kazanır. Newtoncu sezgiden gelen itiraz — *"$10^{17}\text{ kg/m}^3$'lük bir ortam neden çökmez, neden tartılmaz?"* — gizli bir varsayım taşır: kütle yoğunluğunun kendiliğinden çekim alanı kaynakladığı ($\nabla^2\Phi_N=4\pi G\rho_{toplam}$; $\Phi_N$: Newton'un öz-kütleçekim potansiyeli — Ek D · S-28). Evrenakı'da böyle bir yasa yoktur ve olamaz: teoride çekim diye bağımsız bir kuvvet yok; tek alan basınçtır, tek kuvvet gradyandır (Postülat 6). Buradan üç adımlık teorem:
...
Son bir mekanik incelik: kütle *çevresindeki* gradyan bölgesinde ortamın kendisi de tepkisiz değildir — gradyana cevap verir; ama cevabı *düşmek* değildir. **Cevabın biçimi ortamın kohezyonuna bağlıdır:** kohezyonsuz (Euler) bir akışkan dolaşmak zorundadır ($\nabla P/\rho_0=v_\theta^2/r$), ama Evrenakı kohezyonlu olduğundan kuyuyu **gerilerek** — statik kesme dengesiyle — tutabilir ($\tau_{rr}=\rho_n\Phi/2$, kohezyon dayanımının on beş mertebe altında; **Ek M-51**), ve gözlem bu kolu seçer (**Ek M-52**: dört bağımsız gövdede dolaşım dışlandı). Katı deplasman cebi (nükleon) ise akıp dengelenemez; bütün hâlde itilir. **Madde düşer, ortam gerilir.**

Evrenakı'nın bu devasa yoğunluğuna rağmen nesnelerin ölçülebilir bir direnç duymaksızın hareket edebilmesi, atomların içinin "boşluk" olmamasından; tamamen bu sıfıra yakın (ama sıfır olmayan) viskoziteli süper-akışkanla dolu olmasından kaynaklanır. Madde bu okyanusta yüzen yabancı cisim değil, okyanusun kendi içindeki lokal girdapsal yoğunlaşmadır. Hissettiğimiz kütle-itim, bu devasa hidrostatik okyanus basıncındaki küçük dalgalanmalardan (gradyanlardan) ibarettir.
