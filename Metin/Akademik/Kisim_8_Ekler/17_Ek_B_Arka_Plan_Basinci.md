...
*Dürüst kayıt:* (i) $k$ oranı teoride henüz taahhüt edilmemiştir; sonuç bu nedenle yalnızca $O(1)$ düzeyinde belirsizdir (Bölüm 7.4). (ii) Kütleçekim sabiti $G$'nin türetimi (Bölüm 4.2) yalnızca gradyan bağlaşımına ($\alpha$) dayandığından $P_0$'ın mutlak değerinden bağımsızdır — sabitleme onu etkilemez; SN 1987A gecikme bütçesinin (Bölüm 2.4.4) bu değerlerle çapraz kontrolü ise açık iştir (Bölüm 7.4). (iii) **Kalibrasyon kaynağı:** bu sabitlemenin gözlemsel dayanağı artık kütleçekimsel kızıla kayma değil, **ışık bükülmesidir** (2 çarpanını veren gözlem odur; Ek M-42). Kızıla kayma zincirde serbest kalır ve öngörüye döner ($\delta f/f=-\Phi/c^2$); GPS ve Pound–Rebka böylece girdi değil bağımsız doğrulama olur.
