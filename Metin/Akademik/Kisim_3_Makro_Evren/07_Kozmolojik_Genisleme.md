# 3.7 Kozmolojik Genişleme ve Karanlık Enerji Hipotezi
...
## 3.7.2 Atomik Mikro-Girdapların Deşarjı ve Hacim Kazanma

Teorik çerçevemize göre; evrendeki tüm görünür kütleleri (yani hidrojen, helyum, demir vb. tüm atomik çekirdekleri) oluşturan o sonsuz sayıdaki mikro-girdaplar, içinde döndükleri Evrenakı'nın ultra-düşük ama sıfır olmayan viskozitesi (sürtünmesi) nedeniyle, milyarlarca yıllık kozmolojik zaman dilimlerinde rotasyonel enerjilerini çok yavaş da olsa (asimptotik olarak) kaybederler. 

Bu devasa yavaşlama ve enerji aktarımı sürecinde atomik girdaplar (çekirdekler), içsel sirkülasyon dinamikleri gereği çevrelerindeki Evrenakı akışkanını dışarıya doğru sürekli "pompalar" veya "iterler". Evrenakı ortamından aldıkları bu enerjiyi ısı/basınç olarak dışarı salarlar. Evrendeki yüz milyarlarca galaksinin içindeki trilyonlarca kütlenin yarattığı bu kümülatif (toplu) hidrodinamik pompalama (deşarj) etkisi, uzayı dolduran Evrenakı okyanusunun makroskobik olarak **fiziksel hacim kazanmasıyla** (genişlemesiyle) sonuçlanır. 

Bu süreç, bir sıvı kazanının içine atılan sayısız ısıtıcının suyu genleştirmesine, hacmini büyütmesine benzer. Bu bağlamda Evrenakı modeli; evrenin makroskobik genişlemesini açıklamak için ayrı bir Karanlık Enerji (Dark Energy) postülatına ihtiyaç duymaz. Evrenin genişlemesinin motoru, henüz doğrudan gözlemlenememiş bir boşluk enerjisi değil, bizzat kütleyi var eden girdapların (atomların) evrensel zaman ölçeğinde Evrenakı okyanusuna yaptıkları viskoz enerji deşarjının termodinamik bir sonucudur.

> [!IMPORTANT]
> **Bu mekanizmanın hayat şartı: $\eta_E \neq 0$.** Yukarıdaki deşarj *viskoz* bir süreçtir; dolayısıyla bu bölüm, Evrenakı viskozitesinin **sıfır olmadığını** zorunlu kılar. Teorinin "süper-akışkan" nitelemesi bu yüzden asla "tam sürtünmesiz" anlamına gelmez: viskozite sıfıra çok yakındır (bu yüzden gezegen yörüngeleri milyarlarca yıl bozulmaz), ama **kesinlikle sıfır değildir** (bu yüzden kozmolojik ölçekte birikimli deşarj ve dolayısıyla genişleme vardır). $\eta_E = 0$ alınsaydı deşarj debisi de sıfır olur, okyanus hacim kazanmaz ve bu bölümün tamamı — karanlık enerjiye alternatif olma iddiası dahil — geçersizleşirdi.
>
> İki ölçek arasındaki bu ayrım, teorinin sayısal iskeletiyle tutarlıdır: $\eta_E$, resmî parametre envanterinde (Ek C, satır 14) **$0^+$** olarak, yani "sıfıra yakın pozitif" statüsünde kayıtlıdır ve serbest (F) bir parametredir. Aynı $\eta_E$, Satürn halkalarındaki ortam kaynaklı sönüm terimini (Bölüm 3.10.4.2) ve pulsar glitch / YORP öz-hızlanma korelasyonunu (Bölüm 3.1.8) da besler; üç olgu tek bir sonlu viskozite üzerinde kenetlenir. Tam gerekçe için bkz. Bölüm 1.3, 7. postülat kutusu.

## 3.7.3 Kozmolojik Kızıla Kayma ve Yorgun Işık Yanılgısı

Uzak galaksilerden gelen ışığın kırmızıya kayması (Redshift), modern fizikte salt Doppler etkisi veya uzay-zaman dokusunun geometrik olarak uzaması ile açıklanır. Geçmişte bazı alternatif fizik modelleri ise bu durumu açıklamak için "Yorgun Işık" (Tired Light) kavramını, yani ışığın milyarlarca yıllık yolculuğunda sürtünmeyle frekans kaybettiğini öne sürmüştür. 
...
Sonuç olarak; kütleçekimsel kızıla kaymada gördüğümüz o uzaysal **madde ölçeği** oranı ($\Lambda_{kaynak}/\Lambda_{alıcı}$; bkz. 6.2.5 ve Ek M-42), kozmolojik ölçekte bizzat evrenin yaşlanmasından doğan **zamansal ölçek oranına** dönüşür. Uzak galaksilerdeki kızıla kaymanın sebebi ışığın yolda yorulması değil; o ışığın, bugünkü evrene kıyasla çok daha yüksek basınçlı ve hızlı bir geçmişte üretilmiş olmasıdır.
...
1. **İnterferometrik Hız Değişimi:** Evrenakı'nın basıncının ve yoğunluğunun değiştiği derin uzay koşullarında veya çok yoğun kütle sistemlerinin yakın alanında (örneğin Güneş'in taç tabakasına yakın geçişlerde) ışık hızında (lokal $c_{loc}$) hidrodinamiğin öngördüğü yavaşlama/hızlanma saptanamazsa, teori ampirik olarak geçersiz kılınacaktır.
