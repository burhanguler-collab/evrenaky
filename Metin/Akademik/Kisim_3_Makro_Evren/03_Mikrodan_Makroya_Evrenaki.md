...
(function(){
  ...
  const infos = {
    ...
    binary: "<strong>İkili Sistem:</strong> İki kütle birbirine yaklaştığında, ortalarında kalan bölgede Evrenakı itimi çift taraflı gölgelenir ve devasa bir düşük basınç alanı (vakum) oluşur. Dış basınç, içerideki eksik basıncı yenerek kütleleri birbirine iter. (Kütleçekiminin asıl doğası).",
