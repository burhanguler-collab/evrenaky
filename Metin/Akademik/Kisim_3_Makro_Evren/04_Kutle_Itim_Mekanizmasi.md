...
| Michelson–Morley (1887) ve modern **vakum** rezonatörler | Beklenenin $\lesssim1/20$'si; yönsel $\Delta c_0/c_0 < 10^{-17}$ | **Kinematik ölçek terimi ile karşılanır:** madde $\Lambda_{kin}=\sqrt{1-V^2/c_0^2}$ ile kısalır, anizotropi tam sıfırlanır (3.4.6.2, 11.4.8.1). Zarfın tam kavraması gerekmez |
| **Katı dielektrik rezonatörler** (safir WGM, kristal kovuk) | Yönsel $\Delta c_0/c_0<10^{-16}$ | Aynı mekanizmayla karşılanır. Kavrama yoluyla bastırma varsayılsaydı bu satır $1/n^4$ ile zayıf bir kısıt verirdi; $\Lambda_{kin}$ ile **her yol için tam sıfır** çıkar |
...
* **Michelson–Morley.** Sürat, **yerel ortama göre** $c_0=\sqrt{P/\rho}$'dur; laboratuvar çerçevesindeki sürat ise yerel ortamın hızıyla toplanır. Sonucun sıfır çıkması ortamın taşınmasından değil, **düzeneğin kendisinin kısalmasından** gelir: ortama göre $V$ ile giden madde, hareket doğrultusunda $\Lambda_{kin}=\sqrt{1-V^2/c_0^2}$ çarpanıyla kısalır ve saatleri aynı çarpanla yavaşlar (**11.4.8.1**'de sıkıştırılabilir ortamın alan denkleminden türetilir). Kısalma, anizotropiyi tam olarak götürür.
  ...
  >
  > **Bu anizotropi ortam taşınarak değil, madde kısalarak götürülür.** Kinematik ölçek terimi $\Lambda_{kin}=\sqrt{1-V^2/c_0^2}$ cetveli ve saati aynı çarpanla ölçekler; iki yönlü fark her yol için — serbest Evrenakı, hava, katı dielektrik — tam olarak sıfırlanır. Türetim, ortamın sıkıştırılabilir alan denkleminden çıkar ve yeni parametre eklemez (**11.4.8.1**). Sürüklenme zarfının tam kavraması bu satır için **gerekli değildir**; zarfın kalan işi yerel sürükleme bastırmasıdır ve onu M-43'ün altkritik rejimi taşır.
  >
...
Molekül ortamı $u$ ile aynı yöne aktığında, Zerreleri daha seyrek yakalar; molekül çerçevesindeki ritim kızıla kayar ($\omega'=\omega(1-nu/c_0)$) ve kırılma indisi bu kaymış ritimde değerlenir. Entrainment sürüklemesiyle birleşen sonuç *(tam türetim: **Ek M-17**)*:
