...
> **Ön-Tanım — "Zerre" sözcüğünün bu kısımdaki kullanımı:** Kısım 2'de "Zerre", ışığı oluşturan kütleli mermiyi ($m_z$) adlandırdı. Bu kısımda aynı sözcük zaman zaman **Evrenakı ortamının mikroskobik parçacığı** anlamında da geçer (akışkan denklemlerindeki $\rho_z$ "Zerre yoğunluğu" veya Bölüm 3.10'deki "Zerre ortamı" gibi). Karışıklığı önlemek için kural şudur: ışık kastedildiğinde **"Zerre katarı / Zerre mermisi"**, ortam kastedildiğinde **"Evrenakı"** ya da **"Zerre ortamı"** ifadeleri kullanılır; yalın "Zerre", bağlamın açıkça gösterdiği anlamda okunmalıdır.
...
```mermaid
flowchart TD
    KOK["Nükleonun 4 Boyutlu Çift Dönüşü<br/>(Clifford Dönüşü)"]
    KOK --> W1["ω₁ — çok yüksek frekanslı<br/>3B-içi dönüş (Compton düzeyi)"]
    KOK --> W2["ω₂ — W-eksenli<br/>görece yavaş bileşen"]
    W1 --> VORTEKS["Kafesin barındıramadığı bölüm<br/>Evrenakı'na KUSULUR:<br/><b>Makro-Vorteks</b> — yörüngeleri taşıyan girdap (B)"]
    W1 --> SPIN["Kafese kilitli kalıntı:<br/><b>Gövdenin görünür eksen dönüşü</b> (C)"]
    W2 --> POMPA["Doğrusal pulsasyon — akustik pompa:<br/>çevrenin izotropik geri-basıncı =<br/><b>Kütle-İtim (yerçekimi)</b> (A)"]
    W2 --> DEVINIM["Makro dönüşte hizalanan tork eksenleri:<br/><b>Eksen Devinimi</b> — 25.772 yıllık yalpa (C)"]
```
...
Uzay boşluğunda süzülen yalnız asteroitlerin (örneğin *54509 YORP* asteroidi) hiçbir çarpışma yaşamadıkları halde dönüş hızlarının sürekli arttığı gözlemlenmiştir (Lowry vd., 2007). Hatta bazıları o kadar hızlanır ki, merkezkaç kuvvetine dayanamayıp kendi kendilerine parçalanırlar (rotational fission).
...
*Not: Galaktik ölçekte tanımlanan bu $\eta_E$ makro-viskozitesi, Bölüm 3.10'de Satürn halkalarındaki (Mimas 5:3 rezonansı) dikey salınım sönümü bağlamında ele alınan mikroskobik sönüm katsayısı $\eta_z$'nin makroskopik ve kolektif bir tezahürüdür. Her iki parametrenin aynı yapısal direncin farklı ölçeklerdeki tezahürü olduğu varsayımı, iki bağımsız ölçüm (Satürn halkası sönümü ve galaktik dönüş eğrisi) arasında çapraz-doğrulama imkânı sunar; ancak lokal sönüm katsayısından kolektif kuplaj katsayısına geçiş, kendi başına ayrı bir ölçek-geçiş (scale-bridging) modeli gerektirir ve bu, şu an açık bir problemdir.*
