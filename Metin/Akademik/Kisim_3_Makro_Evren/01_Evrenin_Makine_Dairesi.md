...
> **Ön-Tanım — "Zerre" sözcüğünün bu kısımdaki kullanımı:** Kısım 2'de "Zerre", ışığı oluşturan kütleli mermiyi ($m_z$) adlandırdı. Bu kısımda aynı sözcük zaman zaman **Evrenakı ortamının mikroskobik parçacığı** anlamında da geçer (akışkan denklemlerindeki ortam yoğunluğu $\rho$ veya Bölüm 3.10'daki "Zerre ortamı" gibi). Karışıklığı önlemek için kural şudur: ışık kastedildiğinde **"Zerre katarı / Zerre mermisi"**, ortam kastedildiğinde **"Evrenakı"** ya da **"Zerre ortamı"** ifadeleri kullanılır; yalın "Zerre", bağlamın açıkça gösterdiği anlamda okunmalıdır.
...
1. **3B-içi Dönüş ($\omega_1$):** Üç boyutun içindeki bir düzlemde (Bölüm 1.4'teki epür analizinin birinci düzlem grubu) gerçekleşen, parçacığın varlığını sürdürmesini sağlayan asıl enerji girdabıdır; açısal hızı bir varsayım değil, duvar hızı yasasından okunur ($\omega_1 = \sqrt{2}\,c_0/r_n = 5{,}0389\times10^{23}$ rad/s; M-3, bkz. Bölüm 1.4). Üç boyutta **görünür dönüş** üreten bileşen budur.
2. **W-eksenli Dönüş ($\omega_2$):** W eksenini içeren düzlemde (Bölüm 1.4'teki epür analizinin ikinci düzlem grubu) gerçekleşen, 4. boyuttan uzayımıza sarkan bileşendir. Epür analizinin gösterdiği üzere bu bileşen üç boyutta asla dönüş olarak görünmez; tek görünür etkisi, parçacığın eksenini periyodik olarak kaydırmasıdır (**devinim**).
...
* **$\omega_2$'nin makro ürünü: kütle-itimin kendisi.** Pulsasyonun **salınan** payı rastgele fazlar yüzünden $1/\sqrt N$ ile bastırılır ($10^{-26}$ mertebesi) — bu yüzden makro cisim nefes almaz. Ama pulsasyonun **kaynak şiddeti** skaler olduğu için doğrusal toplanır ($Q=Nq_n$) ve durgun bir denge gradyanı kurar: **F1, yani kütle-itim** (1.4.12, Ek M-35). W bileşeninin makro ölçekteki ürünü budur.

  > **Kayıt — devinim bu bileşenin ürünü DEĞİLDİR.** Bu madde daha önce Dünya'nın 25.772 yıllık devinimini *"her atomun içsel 4B yalpalamasının makro sonucu"* sayıyordu; **o okuma geri alınmıştır.** Üç bağımsız sebep: *(i)* ZW dönüşünün hız alanında $v_x=v_y=0$'dır — düzlem X ile Y'ye dokunmaz, ekseni eğemez (1.4.7); *(ii)* nükleonun çift dönüşü M-45'in eş-güç kilidi gereği **tam izoklindir** ($\omega_2=\omega_1$, cebirsel özdeşlik) ve izoklin dönüşün 3B izi saf dönüş artı saf pulsasyondur — eksen hareketi sıfır (1.4.12); *(iii)* içsel bivektörler hizalansa bile W'li bileşenler makro ekseni oynatmaz, yalnız 3B-içi bileşenler oynatır ki onlar klasik kanaldır. Luni-solar devinimi taşıyan şey tork kanalıdır ve 11.7.3 onu bağımsız girdilerle **%0,1** hassasiyetle verir; içkin kanalın payı $\lesssim$**%1**'dir. Teorinin bu olgudaki ayrışan sözü devinimin *varlığı* değil, serbest devinimin **polarizasyonu** ve **sönümsüzlüğüdür** (11.7.6).
...
flowchart TD
    ...
    KOK --> W1["ω₁ — 3B-içi dönüş<br/>duvar hızı yasası: √2c/r = 5,04×10²³ rad/s"]
    KOK --> W2["ω₂ — W-eksenli bileşen<br/>aynı duvar hızı yasası"]
    ...
    W2 --> POMPA["Doğrusal pulsasyon — akustik pompa:<br/>çevrenin izotropik geri-basıncı =<br/><b>Kütle-İtim</b> (A)"]
...
**Kütle, oluştuğu an itibarıyla W-boyutu titreşimleriyle Evrenakı'nı dışlayarak statik kütle-itimini yaratır. Eş zamanlı olarak, 3B-içi bileşeninin büyük bölümünü dışarıdaki Evrenakı'na pompalayarak devasa yörüngesel girdabı (Makro-Vorteks) oluşturur. Kafese kilitli kalan kalıntı rotasyon, gövdenin eksen dönüşünü üretirken, W-bileşeni torkları bu eksene karakteristik devinimini (Precession) işler.** Madde asla durağan bir ağırlık değildir; Evrenakı'nın sonsuz okyanusunda durmaksızın çalışan akustik ve mikrodinamik bir motordur.
...
Coriolis, dönen bir referans sisteminde hareket eden cisimlerde gözlemlenen ve tamamen gözlemcinin çerçevesine bağlı olan bir sözde-kuvvettir (pseudo-force). Eğer Dünya dönmeseydi, Coriolis etkisi olmazdı. Evrenakı'ndaki 4B dönüş ise referans sisteminden tamamen **bağımsız, mutlak ve içsel** bir rotasyondur. Kütle hareketsiz veya dönmüyor bile olsa, içindeki atomların W bileşenindeki mikroskobik dönüşleri sürer — ve makro ölçekte bıraktıkları iz bir yalpalama değil, **kütle-itim alanının kendisidir** (1.4.12). *(Kayıt: bu paragraf daha önce gezegen devinimini bu içsel torka yazıyordu; o okuma geri alınmıştır. Luni-solar devinimi tork kanalı taşır — 11.7.3 onu bağımsız girdilerle %0,1 ile verir — ve içkin kanalın payı $\lesssim$%1'dir. Ayrışma devinimin varlığında değil, **serbest** devinimin polarizasyonunda ve sönümsüzlüğündedir; 11.7.6.)*
