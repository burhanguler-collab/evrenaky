...
*Aday mekanizma notu (ayrıştırılması sıcaklık kontrolü gerektirir):* İki yönlü örüntünün ("önce artış, sonra azalış") teori içindeki adaylarından biri, hareketli plakanın sıkıştırılabilir ortamda (Postülat 1) önünde ittiği **geçici sıkışma cephesidir**: cephe yerel basıncı geçici yükseltir ($c_{loc}$ artar), deplasman havuzu dengeye oturunca kalıcı açık kalır ($c_{loc}$ azalır). Bu aday iki ölçekleme imzası taşır — geçici bileşen plakanın **yaklaşma hızıyla**, kalıcı bileşen plakanın **kütlesiyle** ölçeklenmelidir — ancak sıcaklık kaydı olmadan termal deplasman adayından ayrıştırılamaz. Kütleden bağımsız, ~16.000 m/sn'ye varan taban kaymaları için de başlıca aday termal etkidir (fiberde optik yolun sıcaklık katsayısı ≈$10^{-6}/^\circ$C; bkz. Bölüm 5.1.5). Neden ayrıştırması, sıcaklık-kontrollü tekrar deneyiyle birlikte Bölüm 7.4'te açık hesap kalemi olarak kayıtlıdır.
