...
Bu sonucu şimdi *kullanmayacağız.* Tensörü, izsizliğe hiçbir yerde başvurmadan bileşen bileşen kuracak; sonra izin kendiliğinden sıfır çıktığını göreceğiz.
...
Bunlara mekanizmanın kendisi eklenir: teoride gelgiti yapan şey bir çekme değil, yapısı hesaplanabilen bir artık basınç alanıdır (kuşakta $+\tfrac12$, gelgit ekseninde $-1$). Bu alan klasik kuramda mevcut değildir.
...
> [!IMPORTANT]
> **Ayrım dönme *hızında* değil, kilitlilikte.** Bu, bölümün en kolay yanlış okunan noktasıdır ve peşinen kapatılmalıdır: Güneş'in dönme dönemi (~25,4 gün) ile Ay'ın dönme dönemi (27,32 gün) **birbirinin neredeyse aynısıdır** — açısal hızları yalnız %8 ayrışır. Dolayısıyla "Güneş dönüyor, Ay dönmüyor" cümlesi bir *hız* karşılaştırması olarak okunursa saçmadır: Ay da kendi ekseni etrafında döner, hem de Güneş'le kıyaslanabilir bir hızla.
>
> Teorinin iddiası hızla ilgili değildir: **gelgit kilidi, gövdenin kendi makro-girdabını bastırır** (3.9.1; girdap rekabeti 3.4.4). Kilitli gövdede dönüş, yörünge dolaşımıyla aynı fazda olduğu için ortama göre bağımsız bir sirkülasyon kalmaz ve $\omega_1$ kolunu besleyecek kaynak ortadan kalkar. Serbest dönen gövdede ise kalır. Ayrım bu yüzden **ikili (kilitli / serbest)**, sürekli değil.
>
> **Bu, sınavı zayıflatmaz — kaynağını değiştirir.** 11.1.8 artık şunu sınamaktadır: *kilitlenmenin makro-girdabı bastırdığı iddiası doğru mudur?* İki kaynağın dönme hızları aynı olduğu için tensör yapılarında bulunacak herhangi bir fark, hıza değil **yalnız kilitlilik durumuna** yazılabilir. Karışıklık değişkeni kendiliğinden elenmiştir; diferansiyel deney bu sayede daha temizdir.

**Kuvvet 3 neden tensöre girmiyor?** $\omega_1$ kolu üç kuvvet taşır (3, 4, 5) ama aşağıdaki tabloda yalnız ikisi var; bu bir atlama değil, iki gerekçenin sonucudur. **(i)** M-37 Kuvvet 3 ile Kuvvet 4'ün *aynı vorteksin* teğetsel ve eksenel bileşenleri olduğunu ve ikisinin de tek bir $a_{madde}(R)$ tarafından yönetildiğini türetir; F3'ün radyal gradyan içeriği bu yüzden F4'ün $\beta$'sında **zaten sayılmıştır** — ayrıca eklenmesi çift saymak olurdu. **(ii)** F3'ün kendine ait etkisi sıfırıncı mertebede sürüklemeyi *bastırmak*, birinci mertebede $\Delta v$ ile doğrusal bir **artık kuplaj** uygulamaktır (M-37 Adımlar B). Artık kuplaj bir statik basınç gradyanı değildir; $\eta_E$ üzerinden işler ve zaman ölçeği $\tau_E$'dir. 11.1.9 bu kanalın gelgit periyoduna göre yirmi mertebe yavaş olduğunu göstermiştir — yani F3, statik tensör $T_{ij}=-\frac{1}{\rho_n}\partial_i\partial_jP$'ye ölçülebilir hiçbir katkı yapamaz.
